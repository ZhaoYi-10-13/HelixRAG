...
async def answer_question(request: AnswerRequest):
    """Answer a question using enhanced RAG pipeline with reranking."""
    try:
...
@app.post("/upload", response_model=SeedResponse, tags=["RAG"])
async def upload_files(
    files: List[UploadFile] = File(...),
    file_extensions: Optional[str] = Form(None)
):
    """Upload and process files to add to the knowledge base."""
    try:
        ...
        inserted_count = await rag_service.process_uploaded_files(files)
        
        logger.info(f"File upload completed: {inserted_count} chunks inserted")
        return SeedResponse(inserted=inserted_count)
...
@app.post("/process-directory", response_model=SeedResponse, tags=["RAG"])
async def process_directory(
    directory_path: str = Form(...),
    file_extensions: Optional[str] = Form(None)
):
    """Process all files in a directory to add to the knowledge base."""
    try:
        ...
        # Process directory
        inserted_count = await rag_service.process_directory(directory_path, extensions)
        
        logger.info(f"Directory processing completed: {inserted_count} chunks inserted")
        return SeedResponse(inserted=inserted_count)
